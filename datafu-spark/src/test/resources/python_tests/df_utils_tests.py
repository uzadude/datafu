...
func_dedup_res = df_people.datafu.dedup_with_order(group_col=df_people.id,
                             order_cols=[df_people.age.desc(), df_people.name.desc()])
func_dedup_res.registerTempTable("dedup_with_order")

func_dedupTopN_res = df_people.datafu.dedup_top_n(n=2, group_col=df_people.id,
                                     order_cols=[df_people.age.desc(), df_people.name.desc()])
func_dedupTopN_res.registerTempTable("dedupTopN")

func_dedup2_res = df_people.datafu.dedup_with_combiner(group_col=df_people.id, order_by_col=df_people.age, desc=True,
                               columns_filter=["name"], columns_filter_keep=False)
func_dedup2_res.registerTempTable("dedup_with_combiner")

func_changeSchema_res = df_people.datafu.change_schema(new_scheme=["id1", "name1", "age1"])
